"""Generate an animated demo GIF showing FriendConnect Chat UI and A2UI cards."""
...
def draw_chat_bubble(draw, x, y, text, is_user=False, width=450):
    """Draw a chat bubble."""
    f_msg = get_font(13)
...
def draw_a2ui_match_card(draw, x, y):
    """Draw the A2UI Match Found Card."""
    w = 440
    h = 180
    rect = [(x, y), (x + w, y + h)]
    ...
    draw.text((x + 16, y + 14), "🎉 MATCH FOUND: MAYA", fill=TEXT_PURPLE, font=f_h1)
    draw.text((x + 16, y + 40), "Shared Passion: Pickleball & Culinary Arts", fill=TEXT_WHITE, font=f_body)

    # Placeholder Kawaii Image representation
    draw.rectangle([(x + 16, y + 65), (x + 180, y + 160)], fill=(51, 65, 85))
    draw.text((x + 35, y + 105), "[ Kawaii Artwork ]", fill=TEXT_BLUE, font=f_body)

    # Pair link invitation
    draw.text((x + 200, y + 75), "Pair Link:", fill=TEXT_MUTED, font=f_body)
    draw.text((x + 200, y + 95), "/match/link-8f40a2b", fill=TEXT_BLUE, font=f_link)

    draw.rectangle([(x + 200, y + 125), (x + 380, y + 155)], fill=ACCENT_GRAD_1)
    draw.text((x + 215, y + 133), "Open Live Chat UI →", fill=TEXT_WHITE, font=get_font(11, bold=True))
...
def draw_match_header_banner(draw):
    ...
    rect = [(100, 60), (WIDTH - 100, 110)]
    draw.rectangle(rect, fill=(30, 41, 59), outline=ACCENT_GRAD_2, width=1)

    # Badge
    draw.rectangle([(115, 72), (230, 98)], fill=ACCENT_GRAD_1)
    draw.text((125, 78), "🎉 Match Found!", fill=TEXT_WHITE, font=get_font(11, bold=True))

    f_text = get_font(12)
    draw.text((245, 78), "Pair Link: /match/link-8f40a2b", fill=TEXT_BLUE, font=f_text)

    # Rematch button
    draw.rectangle([(WIDTH - 280, 70), (WIDTH - 115, 100)], fill=ACCENT_GRAD_2)
    draw.text((WIDTH - 270, 78), "Find me another match 🔄", fill=TEXT_WHITE, font=get_font(10, bold=True))
...
def build_frames():
    ...
    draw_chat_bubble(d1, 20, 75, "Hi! I love playing pickleball and cooking gourmet meals!", is_user=True)
    frames.append(img1)

    # Frame 2: Agent intake reply + A2UI Profile Card
    img2 = create_base_canvas()
    d2 = ImageDraw.Draw(img2)
    y2 = draw_chat_bubble(d2, 20, 75, "Hi! I love playing pickleball and cooking gourmet meals!", is_user=True)
    y2 = draw_chat_bubble(d2, 20, y2, "I love it! It sounds like your main vibe is Pickleball & Culinary Arts!", is_user=False)
    ...
    y3 = draw_chat_bubble(d3, 20, 75, "I love it! It sounds like your main vibe is Pickleball & Culinary Arts!", is_user=False)
    y3 = draw_chat_bubble(d3, 20, y3, "Find me friends", is_user=True)
    frames.append(img3)

    # Frame 4: Agent finds match + A2UI Match Card + Celebratory Banner
    img4 = create_base_canvas()
    d4 = ImageDraw.Draw(img4)
    draw_match_header_banner(d4)
    y4 = draw_chat_bubble(d4, 20, 125, "OK, let me find a new match for you!", is_user=False)
    draw_a2ui_match_card(d4, 20, y4)
    frames.append(img4)
    ...
    print(f"Generated optimized demo GIF at: {out_path}")
